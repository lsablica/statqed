...
| `Interval(kind,lower,upper,closure)` | Candidate research shape only: endpoints have the same exact ordered kind (`Integer`, `Rational`, or `Decimal`); `closure` is exactly `closed`, `open`, `left_closed`, or `right_closed`. The latter two names identify the included endpoint. Bounds satisfy `lower < upper`, except `closed` also permits `lower = upper` as a singleton. Equality is structural. | Unsupported. Reversed bounds, empty equal-bound non-closed intervals, unknown closure tokens, mixed endpoint kinds, and IEEE/NaN endpoints are invalid semantic inputs. | None. An untagged array is only an `Array`, not an interval. |
