...
cargo_home=${STATQED_ARROW_CARGO_HOME:-$scratch_root/cargo}
...
"$venv_dir/bin/python" "$probe_root/probe.py" write-altered-file "$exchange_dir/python-altered.arrow"
altered_output="$scratch_root/altered-rust-reader.log"
if cargo run --locked --manifest-path "$probe_root/Cargo.toml" -- read-file "$exchange_dir/python-altered.arrow" >"$altered_output" 2>&1; then
    printf 'Rust foreign reader unexpectedly accepted altered fixture values\n' >&2
    exit 1
else
    grep -F '2002' "$altered_output" >/dev/null || {
        cat "$altered_output" >&2
        printf 'Rust reader failed for a reason other than the expected altered id value\n' >&2
        exit 1
    }
    printf 'rust_rejected_altered_foreign_values=true\n'
fi
