...
- [ ] SQ-0006 implementation evidence — current candidate has five accepted semantic fixtures, 85 retained negatives, three detected deliberate divergences, exact `cddl` 0.10.6 shape checks, two read-only SQ-0005 comparisons plus a direct standards recipe, a 154-package lock-bound license inventory, and permanent static corruption checks. Adversarial/security and versioning reviews approved `d34d35f`; clean-target CI review found the 2 GiB child address-space ceiling was too low for the exact linker, so the resource policy is being corrected and re-reviewed before `IN_REVIEW`.
...
Its scope is one data-free six-field fixture and explicit nonclaims. The final
outcome remains contingent on frozen hashes, distinct security/versioning/CI
and integration review, hosted workflows, and the atomic DONE transition.
