...
Avoid stdin for normative CBOR tests because the CLI auto-detects valid UTF-8 input as JSON. The exact retained single-query OSV request/response for `crates.io:cddl:0.10.6` returned no matching record on 2026-08-10. A separately retained cargo-audit 0.22.2 observation bound to RustSec database commit `309ad29d8fe448bf986019e05d47b9e0e29a2218`, the exact 154-package lock, release/database sources and licenses, tool/archive hashes, and 1,197 loaded advisories reported zero vulnerabilities and zero warnings. Both observations are point-in-time evidence, not a security clearance; the committed request/response, observation, and reproduction checker are under `source-audits/schema/` and `scripts/schema/check_cddl_advisory.py`.
