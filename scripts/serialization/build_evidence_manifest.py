...
def live_protected_files(prefixes: list[str], ignored: list[str]) -> list[dict[str, str]]:
    output: list[dict[str, str]] = []
    ignored_set = set(ignored)
    for prefix in prefixes:
        root = repository_path(prefix)
        if not root.is_dir() or root.is_symlink():
            raise RuntimeError(f"missing or unsafe protected root: {prefix}")
        for path in sorted(root.rglob("*")):
            relative_path = path.relative_to(REPOSITORY)
            if ignored_set.intersection(relative_path.parts):
                continue
            if path.is_symlink():
                raise RuntimeError(f"symlink in live protected baseline: {relative_path}")
            if path.is_file():
                if path.suffix in {".pyc", ".pyo"}:
                    raise RuntimeError(
                        f"generated bytecode in live protected baseline: {relative_path}"
                    )
                output.append(
                    {"path": relative_path.as_posix(), "sha256": sha256(path)}
                )
    unique = {item["path"]: item for item in output}
    return [unique[name] for name in sorted(unique)]
...
def build() -> dict[str, Any]:
    ...
    return {
        ...
        "live_protected_files": live_protected_files(
            live_path_policy["protected_roots"],
            live_path_policy["ignored_directories"],
        ),
